...
public class LaunchObjects : MonoBehaviour
{
    ...
    void LaunchFlag()
    {
        if (Time.time > nextLaunchFlag)
        {
            ...
            float posY = Random.Range(transform.position.y - .5f, transform.position.y + .5f);
    ...
    void LaunchTree()
    {
        if (Time.time > nextLaunchTree)
        {
            float positionX = Random.Range(-transform.localScale.x + .5f, transform.localScale.x - .5f);
            float posX = Random.Range(-maxBorder.x + .5f, maxBorder.x - .5f);
            float posY = Random.Range(transform.position.y - 1.5f, transform.position.y + 1f);
    ...
    void LaunchTreeRock()
    {
        if (Time.time > nextLaunchTreeRock)
        {
            ...
            float posX = Random.Range(-maxBorder.x + .5f, maxBorder.x - .5f);
            float posY = Random.Range(transform.position.y - 2f, transform.position.y + 1f);
